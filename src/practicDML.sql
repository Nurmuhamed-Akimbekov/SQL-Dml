...
insert into people(id, first_name, last_name, email, gender, age, language)
values (101, 'Sultan', 'Isaev', 'sultan@gmail.com', 'Male', 22, 'Kyrgyz');
...
from people;
select round(avg(people.age)) as averge_age
from people;
--7.Найти максимальный возраст среди всех людей
select max(people.age) as max_age
from people
limit 1;
--8. Сумма возрастов всех людей
select sum(people.age) as sum_age
from people;
--9.Получить самого молодого человека(людей)
select min(people.age) as max_age
from people
limit 1;
--Сгруппировать людей по полу и рассчитать средний возраст для каждого
select *
from people
order by age
limit 1;
--10.Сгруппировать людей по полу и рассчитать средний возраст для каждого пола
select round(avg(age)) as averge_age, people.gender
from people
group by gender;
--11.Вывести имя в год рождение людей которые говорят на английском языке
select people.first_name, age
from people
...
select people.last_name, people.email, people.id
from people
where gender = 'Female';
...
select concat(people.first_name, ' ', people.last_name) as full_name, age
from people
where gender = 'Male'
  and age > 23;
...
select people.first_name, people.email
from people
order by email;
...
select people.language, count(*) as speak_language
from people
group by language order by count(*) desc ;
--19.Найти средний возраст для каждого языка, где количество людей больше 2
select people.language, round(avg(people.age))
from people
group by language;
...
--Найти самый распространенный язык среди женщин
select max(language), count(*)
from people
where gender = 'Female';
...
select language, count(*) as group_by_count
from people
group by language;
...
select *
from people
where age not between 24 and 31;
...
from people
where first_name like '%a%'
   or last_name like '%a%' and age between 19 and 55;
